...
def fetch_stats(selected_user, df):
    ...
    media_keywords = ['‎video omitted', '‎image omitted', '‎GIF omitted']
    num_media_msg = df[df['message'].isin(media_keywords)].shape[0]
    ...
    def remove_stop_words(message):
        ...
        for word in message.lower().split():
            if word not in stop_words:
                y.append(word)
        remove_words = ['image', 'vedio', 'omitted', 'deleted.']
        y = (list(filter(lambda x: x not in remove_words, y)))
        return ' '.join(y)
        ...
        for word in message.lower().split():
            if word not in stop_words:
                if(len(word) > 1):
                    words.append(word)
    remove_words = ['image', 'vedio', 'omitted', 'deleted.']
    words = (list(filter(lambda x: x not in remove_words, words)))
